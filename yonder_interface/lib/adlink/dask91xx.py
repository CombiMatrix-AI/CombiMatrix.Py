    ...
    def __init__(self):
        ...
        if "Windows" in self.strOS:
            if '32' in self.strPlatform:
                ...
                self.libHandle = windll.LoadLibrary(str(ROOT_DIR / "resources" / "adlink" / self.MW_DLL_FILE_NAME_X86_WIN))
            else:
                print("System is Windows 64Bits")
                self.libHandle = windll.LoadLibrary(str(ROOT_DIR / "resources" / "adlink" / self.MW_DLL_FILE_NAME_X64_WIN))
        else:
            if '32' in self.strPlatform:
                print("System is Linux 32Bits")
                self.libHandle = cdll.LoadLibrary(str(ROOT_DIR / "resources" / "adlink" / self.MW_DLL_FILE_NAME_X86_LINUX))
            else:
                print("System is Linux 64Bits")
                self.libHandle = cdll.LoadLibrary(str(ROOT_DIR / "resources" / "adlink" / self.MW_DLL_FILE_NAME_X64_LINUX))
